...
def load_windowed_ps_psd(h5_path: str, channel: str = "x"):
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        g = f["spectral"]
        if channel not in g:
            avail = [k for k in g.keys() if k in ("x","y","z")]
...
def spectral_directory_read(
    ...
    for h5_path in h5_pathlist:
        ...
        center_time = start_time + pd.to_timedelta(measure_time / 2.0, unit="s")

        spectrum_list.append(spectrum)
        time_center_list.append(center_time)
...
def _xedges_from_centers(f: np.ndarray) -> np.ndarray:
...
def compute_amplitude_hist_per_frequency(
    ...
    """
    Build a 2D histogram H over amplitude per frequency column (no frequency binning).

    Parameters
    ----------
    spectra_list : list of 1D arrays
        Each array is an averaged spectrum S_i(f) with length Nf (same grid as f).
    f : 1D array
        Common frequency grid (length Nf). All spectra must match this length.
    val_bins : int or array
        If int, number of bins for amplitude; if array, explicit bin edges.
    val_range : (vmin, vmax) or None
        Range for amplitude binning (overrides percentile-based auto).
    log_val : bool
        If True, use log-spaced amplitude bins (strictly positive values kept).
    density : bool
        If True, normalize each per-frequency histogram to integrate to 1.

    Returns
    -------
    H : ndarray, shape (Nf, Ny)
        For each frequency column j, H[j, :] is the histogram over amplitude bins.
...
def plot_amplitude_hist_heatmap(
    ...
    """
    Heatmap of amplitude histograms per frequency (no frequency binning).
    """
    if ax is None:
    ...
    xedges = _xedges_from_centers(f)
    # H is (Nf, Ny); pcolormesh expects (Ny, Nf) when using (xedges,yedges)
    Z = H.T

    norm = LogNorm() if (log_counts and np.any(Z > 0)) else None
    mesh = ax.pcolormesh(xedges, yedges, Z, shading="auto", norm=norm)
    cbar = fig.colorbar(mesh, ax=ax)
    cbar.set_label("Counts" if norm is None else "Counts (log)")

    ax.set_xlabel("Frequency [Hz]")
    ax.set_ylabel("ASD [$\\mu\\mathrm{T}/\\sqrt{\\mathrm{Hz}}$]" if which.lower()=="psd"
...
def plot_2d_spectral_histogram(
    ...
    """
    Make a 2D histogram in (frequency, PS/PSD) space for one channel.

    Returns
    -------
    fig, ax, mesh, H, xedges, yedges
        Matplotlib handles + the raw histogram and bin edges.
    """
    # 1) Load matrices & frequency axis
    ps, psd, seg_center_times, f, meta = load_windowed_ps_psd(h5_path, channel=channel)
    ps = np.sqrt(ps)
    psd = np.sqrt(psd)
