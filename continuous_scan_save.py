...
parser.add_argument('-f', '--formatsave', help='save format [csv / hdf5]', type=str, default='csv')
...
if __name__ == '__main__':
    ...
    t_measure = args.time
    formatsave = args.formatsave

    if not formatsave in ['csv','hdf5']:
        raise Exception("Invalid file format, use csv/hdf5.")
    ...
    if formatsave == 'csv':
        header = ['channel 0', 'channel 1', 'channel 4']
        filename = file_path+f"mag_{timestamp_str}.csv"
        np.savetxt(filename, data_arr,delimiter=',')
        print(f"result saved to {filename}")
    elif formatsave == 'hdf5':
